...
export function StatCard({ title, value, icon: Icon, trend, trendValue, color = 'blue' }) {
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 flex items-start space-x-4 hover-lift transition-all cursor-default">
      <div className={`p-3 rounded-lg shadow-inner ${gradientClass}`}>
        <Icon size={24} />
      </div>
